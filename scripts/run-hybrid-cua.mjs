...
function usage(message) {
	...
	console.error("Usage: npm run cua:hybrid -- [--model provider/model] [--app TextEdit] [--bundle-id com.apple.TextEdit] -- <task>");
	process.exitCode = 2;
}
...
for (let index = 0; index < argv.length;) {
	...
	const modelOption = optionValue(argv, index, "--model");
	if (modelOption) {
		model = modelOption.value;
		index += modelOption.consumed;
		continue;
	}
	...
	usage(`Unknown or incomplete option '${argv[index]}'.`);
	process.exit();
}
...
if (!task) {
	usage();
} else if ([app, bundleId].some((value) => value && /["'\r\n]/.test(value))) {
